    ...
    , ui(new Ui::MainWindow)
{
    ...
    QPixmap logo("D:/Documentos/qt/Ludmila/ProjetoSistemaVersao11/ProjetoSistema11/Image/LudmilaSoftware.png");
    QPixmap Cliente("D:/Documentos/qt/Ludmila/ProjetoSistemaVersao11/ProjetoSistema11/Image/clientes.png");
    QPixmap Produto("D:/Documentos/qt/Ludmila/ProjetoSistemaVersao11/ProjetoSistema11/Image/produtos.png");
    QPixmap Pedido("D:/Documentos/qt/Ludmila/ProjetoSistemaVersao11/ProjetoSistema11/Image/pedidos.png");
...
void MainWindow::on_pushButton_cadastrarCliente_clicked()
{
    try {
        ...
        bool ok = false;
        //verifica se existe outro cliente com esse cpf
        for (int i = 0;i < listaCliente->getSize();i++){
            if(listaCliente->operator[](i).getCpf() == cpf)
                ok = true;
        }
        if(!ok){
...
void MainWindow::on_pushButton_alterarClienteBusca_clicked()
{
    ...
    for(int i = 0;i < listaCliente->getSize() && !ok;i++){
        if(listaCliente->operator[](i).getId() == id){
            ok = true;
            ui->lineEdit_alterarNome->setText(listaCliente->operator[](i).getNome());
            ui->lineEdit_alterarEndereco->setText(listaCliente->operator[](i).getEndereco());
            ui->lineEdit_alterarTelefone->setText(listaCliente->operator[](i).getTelefone());
            ui->lineEdit_alterarCpf->setText(listaCliente->operator[](i).getCpf());
        }
    }
    if(!ok){
...
void MainWindow::on_pushButton_alterarProdutoBusca_clicked()
{
    try{
        ...
        for (int i = 0; i < listaProduto->getSize() && !ok;i++) {
            if(listaProduto->operator[](i).getCodigo() == codigo){
                ok = true;
                ui->lineEdit_alterarDescricao->setText(listaProduto->operator[](i).getDescricao());
                ui->lineEdit_alterarEstoque->setText(QString::number(listaProduto->operator[](i).getQuantidade()));
                ui->lineEdit_alterarPreco->setText(QString::number(listaProduto->operator[](i).getPreco(),'f',2));
            }
        }
        if(!ok){
    ...
    try {
        ...
        if(ok){ //se exite esse produto
            ...
            if(listaProduto->operator[](pos).getQuantidade() < quantidade + aux || quantidade == 0)
                QMessageBox::information(this,"ERRO","Quantidade invalida\nQuantidade em Estoque: " + QString::number(listaProduto->operator[](pos).getQuantidade() - aux));
        ...
        if(listaPedido->empty())
            ...
            for(int i = 0;i < listaPedido->getSize() && !ok && codigo != "";i++){
                if(listaPedido->operator[](i).getIdPedido() == id){
                    ...
                    saida += "\nTotal da Compra: R$ " + QString::number(objPedido.calcularValorTotalCompra(),'f',2) + "\n";
                    saida += "====================================";
                }
            }
            if(!ok)
...
void MainWindow::on_pushButton_ListarPedido_clicked()
{
    try {
        ...
        QString saida = "";
        if(!listaPedido->empty())
            saida += "============ Lista Resumida =======\n";
        for(int i = 0;i < listaPedido->getSize();i++){
            objPedido = listaPedido->operator[](i);
            saida += "\nnumero do pedido: " + QString::number(objPedido.getIdPedido()) + " \n";
            saida += "   -Cliente: " + QString::number(objPedido.getIdCliente()) + " \n";
            saida += "   -Data: " + objPedido.getDataDaCompra() + " \n";
            saida += "   -Total: R$ " + QString::number(objPedido.calcularValorTotalCompra(),'f',2) + "\n";
            saida += "----------------------------------------------------------\n";
...
void MainWindow::on_pushButton_BuscarPedidoListarCliente_clicked()
{
    try {
        if(listaPedido->empty())
            QMessageBox::information(this,"ERRO","Lista Vazia");
        else{
            int Cliente = QInputDialog::getInt(this , "Leitura","Id do Cliente:");
            wictor::Pedido objPedido;
            QString saida = "";
            QString saida2 = "";
            bool ok = false;

            saida += "============ Cliente " + QString::number(Cliente) + " ===========\n";
            for(int i = 0;i < listaPedido->getSize();i++){
                objPedido = listaPedido->operator[](i);
                if(objPedido.getIdCliente() == Cliente){
                    saida2 += "\nnumero do pedido: " + QString::number(objPedido.getIdPedido()) + " \n";
                    saida2 += "   -Cliente: " + QString::number(objPedido.getIdCliente()) + " \n";
                    saida2 += "   -Data: " + objPedido.getDataDaCompra() + " \n";
                    saida2 += "   -Total: R$ " + QString::number(objPedido.calcularValorTotalCompra(),'f',2) + "\n";
                    saida2 += "----------------------------------------------------------\n";
                    ok = true;
                }
            }
